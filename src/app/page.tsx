...
export default function Home() {
  ...
  const fetchUpdates = async () => {
    try {
      ...
      // Prefer static JSON generated by GitHub Actions, then fallback to API route
      const candidates = [
        'https://khanhnc-ctrl.github.io/khanhnc-ctrl-TailorKit-Advisor/updates.json',
        'https://khanhnc-ctrl.github.io/khanhnc-ctrl-TailorKit-Advisor/api/updates'
      ];
      
      let picked = null;
      for (const url of candidates) {
        console.log('Trying updates source:', url);
        try {
          ...
          if (!res.ok) {
            console.log('Source not OK:', url, res.status);
            continue;
          }
          const data = await res.json();
          if (data?.success && Array.isArray(data?.data) && data.data.length > 0) {
            picked = data;
            console.log('Loaded updates from', url);
            break;
          }
        } catch (e) {
          console.log('Source failed:', url, e);
        }
      }
      
      if (picked) {
        console.log('Using RSS data:', picked.data.length, 'articles');
        setUpdates(picked.data);
        setLastUpdated(picked.lastUpdated ?? new Date().toISOString());
      } else {
        console.log('All update sources failed, using fallback');
